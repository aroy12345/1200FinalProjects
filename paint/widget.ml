...
(** TODO: The first part of task 5 requires you to implement the following
  ...
  type 'a listen = {
 mutable contents : ('a->unit) list;
 mutable value : 'a
}

let rec iterate(value: 'a)(values: ('a->unit) list): unit =
      begin match values with 
      | [] -> ()
      | head:: tail -> head(value); iterate(value)(tail);
      end
...
let make_controller (v: 'a) : 'a value_controller =
  let listener: 'a listen = {contents = []; value = v} in
  listener.contents <- [fun(x:'a) -> listener.value<-x];
  {
   add_change_listener = (fun(newl:('a-> unit )) -> 
   listener.contents<-newl :: listener.contents);
   get_value = (fun () -> listener.value);
   change_value = (fun(x:'a) -> iterate(x)(listener.contents));
  }
...
let checkbox (init: bool) (s: string) : widget * bool value_controller =
     ...
     let(display, lc2) = label ("Thick Lines") 
     in
     let (w,lc,nc) = button s in
     let controller = make_controller(value.contents) in 
     nc.add_event_listener(mouseclick_listener(fun()->value.contents
     <- not(value.contents); controller.change_value(value.contents); 
      if(value.contents) 
      then (lc.set_label(" X "); 
      lc2.set_label("Thick Lines"))
      else (lc.set_label("    "); 
      lc2.set_label("Thin Lines"))));
    (border(vpair(vpair(space(1,1))(hlist [space(1,1);border(w); space(5,5);
     display]))(space(1,1))), 
    controller);
